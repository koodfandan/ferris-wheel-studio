...
export function PartsDrawer({
  ...
  const speciesMeta =
    SPECIES_OPTIONS.find((item) => item.id === recipe.species) ?? SPECIES_OPTIONS[0];
  ...
  const railTabs: Array<{ id: RightRailTab; label: string }> = [
    ...
    { id: "collection", label: locale === "zh" ? "收藏" : "Build" },
    { id: "history", label: locale === "zh" ? "历史" : "History" },
  ];

  if (selectedSlot === "palette") {
    return (
      ...
      <div className="parts-drawer-shell">
        <nav className="parts-drawer-tabs parts-drawer-tabs-vertical" aria-label={locale === "zh" ? "右侧导航" : "Right rail tabs"}>
          {railTabs.map((tab) => (
            <button
              ...
              title={tab.label}
            >
              <span className="parts-tab-icon" aria-hidden="true">{RAIL_TAB_ICONS[tab.id]}</span>
        ...
        {activeTab === "parts" ? (
          <>
            <DrawerHeader
              title={copy.paletteLibrary}
              blurb={copy.paletteLibraryBlurb}
  ...
  const options = getSlotOptions(recipe.species, selectedSlot as PartSlot);

  return (
    <aside className="parts-drawer">
      <nav className="parts-drawer-tabs" aria-label={locale === "zh" ? "右侧导航" : "Right rail tabs"}>
        {railTabs.map((tab) => (
          <button
            key={tab.id}
            type="button"
      ...
      {activeTab === "parts" ? (
        <>
          <DrawerHeader
            title={getLocalizedSlot(selectedMeta.id, locale)?.label.zh ?? selectedMeta.label}
            blurb={getLocalizedSlot(selectedMeta.id, locale)?.blurb.zh ?? selectedMeta.blurb}
            extra={getLocalizedSpecies(recipe.species, locale)?.label.zh ?? speciesMeta.label}
            locale={locale}
          />
          {blockedCount ? (
            <div className="parts-guard-banner">
              <strong>{locale === "zh" ? `${blockedCount} ${copy.blockedOptions}` : `${blockedCount} ${copy.blockedOptions}`}</strong>
      ...
      {activeTab === "status" ? (
        <>
          ...
          <div className="drawer-footer-card">
            ...
            <small>{copy.currentSelection}: {getLocalizedRecipeLabel(recipe, selectedSlot, locale)}</small>
          </div>
        </>
      ) : null}
